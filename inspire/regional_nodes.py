...
class IPAdapterConditioning:
    def __init__(self, mask, weight, weight_type, noise=None, image=None, embeds=None, start_at=0.0, end_at=1.0, unfold_batch=False,
                 ipadapter_pipe_opt=None, faceid_v2=False, weight_v2=False):
        self.mask = mask
        self.image = image
        self.embeds = embeds
        self.weight = weight
        ...
        self.ipadapter_pipe_opt = ipadapter_pipe_opt

    def doit(self, ipadapter_pipe):
        if self.ipadapter_pipe_opt is None:
            ipadapter, model, clip_vision, insightface = ipadapter_pipe
        else:
            ipadapter, model, clip_vision, insightface = self.ipadapter_pipe_opt
...
class RegionalIPAdapterMask:
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                ...
                "ipadapter_pipe_opt": ("IPADAPTER_PIPE",)
            }
        }
    ...
    def doit(self, mask, image, weight, noise, weight_type, start_at=0.0, end_at=1.0, unfold_batch=False, faceid_v2=False, weight_v2=False, ipadapter_pipe_opt=None):
        cond = IPAdapterConditioning(mask, weight, weight_type, noise=noise, image=image, start_at=start_at, end_at=end_at, unfold_batch=unfold_batch,
                                     ipadapter_pipe_opt=ipadapter_pipe_opt, faceid_v2=faceid_v2, weight_v2=weight_v2)
        return (cond, )
...
class RegionalIPAdapterColorMask:
    ...
    def doit(self, color_mask, mask_color, image, weight, noise, weight_type, start_at=0.0, end_at=1.0, unfold_batch=False, faceid_v2=False, weight_v2=False, ipadapter_pipe_opt=None):
        mask = color_to_mask(color_mask, mask_color)
        cond = IPAdapterConditioning(mask, weight, weight_type, noise=noise, image=image, start_at=start_at, end_at=end_at, unfold_batch=unfold_batch,
                                     ipadapter_pipe_opt=ipadapter_pipe_opt, faceid_v2=faceid_v2, weight_v2=weight_v2)
        return (cond, mask)
